...
def test_t_beam_all_sizes_against_published():
...
def test_t_beam_depths_reconcile_with_published_stack():
...
def test_t_beam_web_height_subgroups():
    assert IeTBeamSection("T1").dimensions.web_height == 85.0
    assert IeTBeamSection("T3").dimensions.web_height == 240.0


def test_t_beam_invalid_size_raises():
    with pytest.raises(ValueError):
        IeTBeamSection("T11")


def test_ye_beam_all_sizes_against_published():
...
def test_ye_beam_vertical_face_position():
...
def test_ye_beam_invalid_size_raises():
    with pytest.raises(ValueError):
        IeYEBeamSection("YE9")

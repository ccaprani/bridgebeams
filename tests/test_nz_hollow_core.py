...
@pytest.mark.parametrize('unit,nvoid,key_sides',[('inner',2,2),('outer',1,1)])
def test_independent_area_and_voids(unit,nvoid,key_sides):
...
@pytest.mark.parametrize('kwargs',[{'depth':651},{'depth':650,'unit':'outer'},{'unit':'edge'},{'circle_points':31},{'circle_points':33},{'circle_points':True}])
def test_invalid_requests(kwargs):
    with pytest.raises(ValueError): NzHollowCoreSection(**kwargs)


@pytest.mark.parametrize('depth,bottom,void_height',[(650,130,380),(900,155,605)])
def test_single_inner_dimension_chain(depth,bottom,void_height):

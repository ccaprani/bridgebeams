...
def test_all_sizes_match_attested_table():
...
def test_constant_widths_attested():
...
def test_t15_t18_share_cross_section():
...
def test_t22_does_not_exist():
    with pytest.raises(ValueError):
        MostostalTSection("T22")


def test_polygon_valid_symmetric_closed():
    for size in MostostalTSection.SIZES:
...
def test_ixx_matches_closed_form():
    for size in MostostalTSection.SIZES:
        beam = MostostalTSection(size)
...
def test_web_widens_downward():
    for size in MostostalTSection.SIZES:
        d = MostostalTSection(size).dimensions
        assert d.web_bottom_width > d.web_top_width, size


def test_t27_web_bottom_attested():

...
- Full test suite after the geometry changes: **314 passed**, 14 existing
...
- Wheel checked outside the checkout: **all 24 family JSON tables included**;
  all 46 profiles added after visual review construct from the extracted wheel.
  The preceding 43-profile expansion was checked separately.
...
UK and Ireland are separate country entries. Irish producer profiles belong
to `bridgebeams.ie`; `bridgebeams.uk` currently exports no profiles. Do not
label Ireland/UK as a shared jurisdiction or double-count Irish profiles in
UK totals. The114-jurisdiction research footprint includes access-blocked
...
`tools/build_coverage.py`. Current totals:284 fixed profiles in14 countries,
114 research jurisdictions,159 source records; default table of 117includes three
legacy-only implementation/template countries (BE/GR/PL). The helper

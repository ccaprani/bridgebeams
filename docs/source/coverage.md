...
The September multilingual discovery search now includes Pakistan NHA Types A–H, the PCI AASHTO I–VI reference, AASHTO-named leads in the Philippines, Honduras, Nicaragua, Guatemala, El Salvador and Peru, and follow-up records for Korea and Argentina. The NHAI NH 45-A drawing supplies India's first fully dimensioned project midspan profile; its feasibility-report status is shown in the family documentation. Country details on the map show original source links, local-language titles, drawing locators and section names where available. A blocked lead's indexed terms are explicitly unverified.
